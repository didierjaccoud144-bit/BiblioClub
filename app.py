...
# --- 1. BIBLIOTHÈQUE ---
with onglets[0]:
    recherche = st.text_input("🔍 Rechercher un livre ou un auteur...", "").lower()
        ...
        with st.container():
            ...
            with c2:
                ...
                if row.get(COL['Avis_Lecteurs']):
                    with st.expander("💬 Voir les avis des lecteurs"): st.markdown(row[COL['Avis_Lecteurs']])
                
                with st.expander("📝 Donner mon avis"):
...
with onglets[2]:
    ...
    st.markdown("### 📦 Mes livres en voyage")
    ...
    if not mes_prets.empty:
        ...
        recap[COL["Statut"]] = recap[COL["Statut"]].replace({"Demandé": "⏳ En attente", "Emprunté": "📕 Chez l'emprunteur"})
        st.table(recap)
    else: st.info("Tous tes livres sont chez toi ! 🏠")
    ...
    with st.form("sugg"):
        ...
        if st.form_submit_button("WhatsApp"):
            st.link_button("📱 Envoyer", envoyer_whatsapp(f"Hello, je suggère d'ajouter {s} !"))
    st.write("---")
    st.markdown("#### 📚 Ma collection complète")
    mes_l = df_livres[df_livres[COL["Proprio"]] == utilisateur]
    for idx, r in mes_l.iterrows():
        stat_lbl = "⏳ Demandé" if r[COL['Statut']] == "Demandé" else r[COL['Statut']]
        with st.expander(f"📙 {r[COL['Titre']]} ({stat_lbl})"):
...
with onglets[3]:
    ...
    if mode == "✅ Manuel":
        with st.form("add"):
            t, a, n, c = st.text_input("Titre"), st.text_input("Auteur"), st.select_slider("Ma note", ["📚","📚📚","📚📚📚","📚📚📚📚"]), st.text_area("Mon Avis")
...
# --- DERNIER ONGLET : GUIDE COMPLET DÉTAILLÉ ---
with onglets[idx_guide]:
    st.subheader("📖 Guide complet du membre")
    
    with st.expander("📱 1. Installation sur téléphone", expanded=True):
        st.markdown("""
    ...
    with st.expander("🔍 2. Emprunter un livre"):
        ...
        * ⏳ **Sablier** : Demande envoyée ! Le propriétaire doit valider.
        * 📕 **Livre Rouge** : Déjà en prêt chez quelqu'un.
        """)

    with st.expander("🤝 3. Gérer mes prêts (Proprio)"):
        st.markdown("""
        * Dès qu'un membre veut un de tes livres, un **bandeau orange** t'avertit.
        * Va dans l'onglet **Emprunts** pour **✅ Valider** ou **❌ Décliner**.
        * Une fois le livre rendu physiquement, clique sur **🔄 Rendu** pour le libérer.
        """)

    with st.expander("💬 4. Donner mon avis"):
        st.markdown("""
        * Sous chaque livre, clique sur **"Donner mon avis"**.
        * Ton retour sera visible par tout le club dans la section **"Avis des lecteurs"**.
        """)
