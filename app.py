...
# Connexion directe (Testée et approuvée !)
def get_sheets():
...
try:
    ...
    st.error(f"Connexion Google Sheets interrompue. Cliquez sur Actualiser. ({e})")
    st.stop()
...
# --- 1. BIBLIOTHÈQUE ---
with onglets[0]:
    rech = st.text_input("🔍 Rechercher (Titre, Auteur, Catégorie)...").lower()
    tri = st.selectbox("Trier par", ["Derniers ajouts", "Note", "Titre (A-Z)", "📗 Disponible uniquement"])
    df_f = df_livres.copy()
    if tri == "📗 Disponible uniquement": df_f = df_f[df_f[COL["Statut"]] == "Libre"]
    if rech:
    ...
    if tri == "Titre (A-Z)": df_f = df_f.sort_values(by=COL["Titre"])
    elif tri == "Note": df_f = df_f.sort_values(by=COL["Note"], ascending=False)
    else: df_f = df_f.iloc[::-1]
        ...
        with st.container():
            ...
            with col_a:
                ...
                with st.expander("💬 Avis"):
                    n_l = st.select_slider("Note", ["📚","📚📚","📚📚📚","📚📚📚📚"], key=f"n_{idx}")
            ...
            with col_b:
                ...
                if r[COL['Avis_Lecteurs']]:
                    with st.expander("💬 Voir les avis"): st.markdown(r[COL['Avis_Lecteurs']])
            st.markdown("---")
...
with onglets[2]:
    ...
    s_p = st.text_input("🔍 Rechercher dans mon historique...", "").lower()
    ...
    if s_p: mes_p = mes_p[mes_p[COL["Titre"]].astype(str).str.lower().str.contains(s_p) | mes_p[COL["Auteur"]].astype(str).str.lower().str.contains(s_p)]
    for idx, r in mes_p.iterrows():
        c1, c2, c3 = st.columns([3, 1.5, 1])
        c1.write(f"**{r[COL['Titre']]}**\n({r[COL['Emprunteur']]})")
        c2.write(f"{r[COL['Statut']]}")
        with c3:
            if st.button("🔄 Rendu", key=f"rendu_{idx}"):
                sh_l, _ = get_sheets(); real_idx = df_livres.index[df_livres[COL['Titre']] == r[COL['Titre']]][0] + 2
    ...
    st.subheader("🛠️ Support & Suggestions")
    msg_s = st.text_area("Votre message pour Didier")
    type_s = st.selectbox("Type", ["Suggestion", "Bug"])
    c_m, c_w = st.columns(2)
    with c_m: st.link_button("📧 Mail", generer_lien_mail(f"MeliMelo - {type_s}", f"De {utilisateur}: {msg_s}"))
        ...
        if st.form_submit_button("Valider l'ajout"):
            if t and a:
                with st.spinner("Envoi..."):
                    sh_l, _ = get_sheets()
                    sh_l.append_row([t, a, utilisateur, avis, "Libre", "", note, datetime.now().strftime("%Y-%m-%d"), "", cat])
                    st.success(f"✅ '{t}' ajouté !"); st.balloons()
...
with onglets[4]:
    ...
    else: st.write("Réservé à Didier et Amélie.")
...
with onglets[5]:
    ...
    with st.expander("🔐 2. Connexion"):
        st.markdown("Identifiez-vous avec votre Prénom et votre Code Secret (4 caractères min).")
    with st.expander("🔍 3. Exploration"):
        st.markdown("Cherchez par Titre, Auteur ou Catégorie. 📗=Disponible, ⏳=En attente, 📕=Déjà prêté.")
    with st.expander("🤝 4. Prêts & Emprunts"):
    ...
    with st.expander("💬 5. Avis & Notes"):
        st.markdown("Notez vos lectures pour conseiller les autres membres !")
