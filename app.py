...
# On ajoute une option "Vide" pour la note 0
LISTE_NOTES = ["❌", "📚", "📚📚", "📚📚📚", "📚📚📚📚"]
...
# HEADER AVEC IMAGE CORRIGÉE
c1, c2 = st.columns([1, 4])
with c1:
    st.image("https://raw.githubusercontent.com/didierjaccoud144-bit/BiblioClub/main/image_3.png", width=80)
with c2:
...
# --- 1. BIBLIOTHÈQUE (RÉTABLISSEMENT AVIS) ---
with onglets[0]:
    ...
    for idx, r in df_f.iloc[::-1].iterrows():
        ...
        with st.container():
            ...
            with col_a:
                ...
                # RÉTABLISSEMENT DU BLOC AVIS MEMBRE
                with st.expander("💬 Ajouter un avis"):
                    n_l = st.select_slider("Ma Note", options=LISTE_NOTES, key=f"n_{idx}")
                    c_l = st.text_area("Mon retour", key=f"c_{idx}")
                    if st.button("Publier l'avis", key=f"p_{idx}"):
                        sh_l, _ = get_sheets(); real_idx = int(df_livres.index[df_livres[COL['Titre']] == r[COL['Titre']]][0] + 2)
                        nouveau_bloc = (str(r[COL["Avis_Lecteurs"]]) + f"\n\n**{utilisateur}** ({n_l}) : {c_l}").strip()
                        sh_l.update_cell(real_idx, 9, nouveau_bloc); refresh()
            
            with col_b:
                # RÉTABLISSEMENT AVIS PROPRIO
                if r[COL['Avis']]: st.caption(f"⭐ **Proprio :** {r[COL['Avis']]}")
                if r[COL['Avis_Lecteurs']]:
                    with st.expander("💬 Voir les avis des lecteurs"): st.markdown(r[COL['Avis_Lecteurs']])
            st.markdown("---")
...
with onglets[5]:
    ...
    with st.expander("🔍 3. Exploration & Recherche"):
        st.markdown("📗=Disponible, ⏳=En attente, 📕=Déjà prêté.")
    with st.expander("🤝 4. Emprunts & Prêts"):
        st.markdown("Une fois rendu, le proprio clique sur 🔄 Rendu dans son profil.")
    with st.expander("💬 5. Avis & Notes"):
        st.markdown("Notez et commentez vos lectures !")
    with st.expander("👤 6. Profil & Support"):
        st.markdown("Gérez vos livres et utilisez le bouton 🛠️ Support.")
